...
func (r *GatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	kengineEps, err := r.getEndpoints(ctx, gw)
	if err != nil {
		return ctrl.Result{}, err
	}
	if len(kengineEps.Subsets) < 1 {
		return ctrl.Result{}, errors.New("no endpoint subsets found for gateway service")
	}

	// Configure Kengine in parallel, so when someone runs Kengine as a DaemonSet on
	// a 5,000 node cluster, we bring the gateway controller to its knees.
	var wg sync.WaitGroup
	for _, a := range kengineEps.Subsets[0].Addresses {
		if a.TargetRef == nil {
			// TODO: log error
			continue
		}
		...
		go func(a corev1.EndpointAddress) {
			...
			log.V(1).Info("Programming Kengine instance", "ip", a.IP, "target", target)
			// TODO: configurable scheme and port
			url := "https://" + net.JoinHostPort(a.IP, "2021") + "/load"
			req, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewBuffer(b))
			if err != nil {
				log.Error(err, "Error programming Kengine instance", "ip", a.IP, "target", target)
				return
			}
			...
			if res.StatusCode != http.StatusOK {
				...
				log.Error(errors.New(string(b)), "Error programming Kengine instance", "status_code", res.StatusCode, "ip", a.IP, "target", target)
				return
			}
			log.V(1).Info("Successfully programmed Kengine instance", "ip", a.IP, "target", target)
		}(a)
	}
	wg.Wait()
