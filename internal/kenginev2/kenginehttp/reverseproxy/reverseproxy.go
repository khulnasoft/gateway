...
type Handler struct {
	...
	FlushInterval kengine.Duration `json:"flush_interval,omitempty"`
	...
	// Headers manipulates headers between Kengine and the backend.
	...
	StreamTimeout kengine.Duration `json:"stream_timeout,omitempty"`
	...
	// enabling this prevents streams from closing when Kengine's config
	...
	StreamCloseDelay kengine.Duration `json:"stream_close_delay,omitempty"`
...
type LoadBalancing struct {
	...
	TryDuration kengine.Duration `json:"try_duration,omitempty"`
	...
	TryInterval kengine.Duration `json:"try_interval,omitempty"`
	...
	RetryMatch []kenginehttp.Match `json:"retry_match,omitempty"`
}
