...
import (
	...
	"github.com/khulnasoft/gateway/internal/kenginev2/kenginehttp/headers"
	"github.com/khulnasoft/gateway/internal/kenginev2/kenginehttp/reverseproxy"
	"github.com/khulnasoft/gateway/internal/kenginev2/kenginehttp/rewrite"
	"github.com/khulnasoft/gateway/internal/kenginev2/kenginetls"
)

func (i *Input) getHTTPServer(s *kenginehttp.Server, l gatewayv1.Listener) (*kenginehttp.Server, error) {
	var hostname string
	if l.Hostname != nil {
	...
	for _, hr := range i.HTTPRoutes {
		...
		terminal := false
		matchers := []kenginehttp.Match{}
		handlers := []kenginehttp.Handler{}
		...
		if len(hr.Spec.Hostnames) > 0 {
			...
			matcher := kenginehttp.Match{
				Host: make(kenginehttp.MatchHost, len(hr.Spec.Hostnames)),
			}
		...
		for _, rule := range hr.Spec.Rules {
			...
			ruleHandlers := []kenginehttp.Handler{}
			for _, f := range rule.Filters {
				var handler kenginehttp.Handler
				switch f.Type {
				...
				case gatewayv1.HTTPRouteFilterRequestRedirect:
					...
					if v.Scheme != nil {
						...
						// Keep the scheme the same (this is a Kengine placeholder).
					...
					if v.Hostname != nil {
						...
						// Keep the hostname the same (this is a Kengine placeholder).
					...
					if v.Path != nil {
						...
						// Keep the path the same (this is a Kengine placeholder).
					...
					handler = &kenginehttp.StaticResponse{
						Headers: http.Header{
							textproto.CanonicalMIMEHeaderKey("Location"): {location.String()},
						},
						StatusCode: kenginehttp.WeakString(strconv.Itoa(statusCode)),
					}

					// TODO: this is what kengine does for a `redir` directive,
				...
				case gatewayv1.HTTPRouteFilterURLRewrite:
					...
					//
					// The other option is to implement a custom handler in kengine
					...
					if v.Path != nil {
						...
						case gatewayv1.PrefixMatchHTTPPathModifier:
							...
							//
							// I'm unsure how to map this to Kengine as it seems like
							// we need to know the request path in order to replace the prefix.
							// ref; https://khulnasoft.com/docs/kenginefile/directives/uri#examples
							//
							// We may be able to take advantage of URI placeholders.
							// ref; https://khulnasoft.com/docs/json/apps/http/#docs

							replacement := *p.ReplacePrefixMatch

							// Kengine-specific: if the replacement is `/`, use the
				...
				case gatewayv1.HTTPRouteFilterRequestMirror:
					...
					// This will require us to build a custom Kengine module if we
					// want request mirroring.
					// ref; https://github.com/khulnasoft/kengine/issues/4211
					//
					// TODO: implement
			...
			if len(rule.BackendRefs) > 0 {
				for _, bf := range rule.BackendRefs {
					...
					// Kengine to use TLS when connecting to the backend, just like
					...
					if bTLSPolicy.Name != "" {
						...
						if len(policy.CACertificateRefs) > 0 {
							...
							tls.CA = kenginetls.InlineCAPool{
								TrustedCACerts: certs,
							}
						}
						// Kengine will default to using system trust for TLS if
						...
						case "kubernetes.io/ws":
							...
							//
							// Kengine automatically proxies WebSockets without any additional
			...
			if !matcher.IsEmpty() {
				handlers = append(handlers, &kenginehttp.Subroute{
					Routes: []kenginehttp.Route{
						{
							MatcherSets: []kenginehttp.Match{*matcher},
		...
		// Add the route.
		routes = append(routes, kenginehttp.Route{
	...
	//
	// Passthrough requires using a Layer 4 TLS listener with Kengine, so it is
	// handled separately.
	if l.TLS == nil {
	...
	if hostname != "" {
		...
		s.TLSConnPolicies = append(s.TLSConnPolicies, &kenginetls.ConnectionPolicy{
			Matchers: kengine.ModuleMap{
				"sni": snis,
			},
		})
	}
