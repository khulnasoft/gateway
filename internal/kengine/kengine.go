...
// Config represents the configuration for a Kengine server.
type Config struct {
	Admin   *kenginev2.AdminConfig `json:"admin,omitempty"`
	Logging *kenginev2.Logging     `json:"logging,omitempty"`
	Apps    *Apps                  `json:"apps,omitempty"`
}

// Apps is the configuration for "apps" on a Kengine server.
type Apps struct {
	HTTP   *kenginehttp.App `json:"http,omitempty"`
	TLS    *kenginetls.TLS  `json:"tls,omitempty"`
	Layer4 *layer4.App      `json:"layer4,omitempty"`
}

// Input is provided to us by the Gateway Controller and is used to
// generate a configuration for Kengine.
type Input struct {
	...
	httpServers   map[string]*kenginehttp.Server
	layer4Servers map[string]*layer4.Server
	config        *Config
	loadPems      []kenginetls.CertKeyPEMPair
}

// Config generates a JSON config for use with a Kengine server.
func (i *Input) Config() ([]byte, error) {
	i.httpServers = map[string]*kenginehttp.Server{}
	i.layer4Servers = map[string]*layer4.Server{}
	i.config = &Config{
		Admin: &kenginev2.AdminConfig{Listen: ":2019"},
		Apps:  &Apps{},
	}
	...
	if len(i.httpServers) > 0 {
		for _, s := range i.httpServers {
			...
			s.Routes = append(s.Routes, kenginehttp.Route{
				Handlers: []kenginehttp.Handler{
					&kenginehttp.StaticResponse{
						Close:      true,
						StatusCode: kenginehttp.WeakString(strconv.Itoa(http.StatusMisdirectedRequest)),
						Body:       "unable to route request\n",
						Headers: http.Header{
							"Kengine-Instance": {"{system.hostname}"},
						},
					},
				},
				Terminal: true,
		...
		i.config.Apps.HTTP = &kenginehttp.App{
			Servers: i.httpServers,
			// TODO: make this user configurable.
			// This is used to allow us to ensure the config reloads in a reasonable
			// amount of time. Without it, Kengine will wait "indefinitely" which
			// is not what we want to happen.
			GracePeriod: kenginev2.Duration(15 * time.Second),
		}
	}
	...
	if len(i.loadPems) > 0 {
		i.config.Apps.TLS = &kenginetls.TLS{
			Certificates: &kenginetls.Certificates{
				LoadPEM: i.loadPems,
			},
...
func (i *Input) handleHTTPListener(l gatewayv1.Listener) error {
	...
	if !ok {
		s = &kenginehttp.Server{
			...
			// Enable metrics on the server, metrics are scraped via the Kengine admin
			// endpoint.
			Metrics: &kenginehttp.Metrics{},

			// Handle errors.
			Errors: &kenginehttp.HTTPErrorConfig{
